...
void ocl::DisplayContextInfo(cl_context context)
{
	...
	std::unique_ptr<cl_device_id[]> devices(new cl_device_id[deviceCount]);
	clGetContextInfo(context, CL_CONTEXT_DEVICES, sizeof(cl_device_id) * deviceCount, devices.get(), NULL);
	for (cl_uint i = 0; i < deviceCount; ++i) {
		...
		clGetDeviceInfo(devices[i], CL_DEVICE_NAME, size, name.get(), NULL);
